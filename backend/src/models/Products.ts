...
productSchema.methods.addToStock = async function(quantity: number): Promise<void> {
  this.stock += quantity;
  await this.save();
};

// Instance method to reduce stock
productSchema.methods.reduceStock = async function(quantity: number): Promise<void> {
